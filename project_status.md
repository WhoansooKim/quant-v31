...
> ## 🔴 2026-08-18 정정 — 위 §22.AL 수치는 유령손실 버그로 오염된 값임
>
> 위 표는 `runner.py` 의 **유령 손실 버그**(시세 없는 날 보유 포지션 평가액을 0 처리, §22.AO-3 결함④)
> 가 남아 있던 상태에서 뽑은 것이다. 버그 수정 후 동일 스크립트(`scripts/backtest_market_filter.py`)
> 를 재실행한 결과 **결론이 정반대로 뒤집혔다.**
>
> | 구간 | 필터 | 진입 | 총수익 | Sharpe | MDD | 승률 |
> |---|---|---|---|---|---|---|
> | **전체 22-25** | OFF | 634 | +64.9% | 1.08 | −13.8% | 37% |
> | | **ON** | 497 | **+103.1%** | **1.45** | **−12.0%** | 38% |
> | **약세장 2022** | OFF | 139 | −4.0% | −0.30 | −11.9% | 21% |
> | | **ON** | 58 | **−0.5%** | −0.03 | **−4.4%** | 25% |
> | **강세장 23-24** | OFF | 316 | +46.7% | 1.55 | −9.1% | 41% |
> | | **ON** | 289 | **+74.7%** | **1.98** | **−9.0%** | 38% |
>
> **정정 내용:**
> - §22.AL 은 "필터 ON 이 전체 Sharpe 소폭↓(1.41→1.39), 강세장 기회비용(+65.4→+57.4%)이 방어이득을
>   거의 상쇄 — **방어장치이지 알파원천 아님**" 으로 결론냈다. **이 결론은 철회한다.**
> - 재검증에서 필터 ON 은 **세 구간 모두에서 우위**다. 전체 수익 **+38.2%p**, Sharpe 1.08→1.45.
>   **강세장 기회비용은 존재하지 않았다** — 오히려 +46.7% → +74.7% 로 28%p 앞선다.
> - 약세장 방어는 방향은 같으나 격차가 더 크다: 손실 −4.0% → **−0.5%**(1/8), MDD −11.9% → **−4.4%**(63%↓).
> - 버그가 **필터 ON 쪽을 더 심하게 깎고 있었다.** ON 은 하락 국면에 현금을 들고 있어 미청산 포지션
>   구성이 달랐고, 결측 종목의 평가액 소실 영향을 다르게 받았기 때문으로 추정.
>
> **⚠️ 재검증 수치도 그대로 믿지 말 것:**
> - 필터 OFF 전체수익이 §22.AL +94.4% → 재검증 +64.9%, 강세장 +65.4% → +46.7% 로 **양쪽 다 낮아졌다.**
>   버그 수정만으로는 설명되지 않으며, 유니버스 구성 변화·데이터 갱신이 섞였을 수 있다.
> - 강세장에서 진입 27건(316→289, 8.5%)을 막았을 뿐인데 수익이 28%p 오른 것은 **폭이 과하다.**
>   자본이 공유되므로 나쁜 진입을 피하면 현금이 보존돼 복리로 증폭되는 구조이긴 하나,
>   단일 유니버스·생존편향(현재 종목을 과거 적용) 위의 수치임을 감안해야 한다.
> - **운영상 조치는 불필요** — `market_filter_enabled=true` 로 이미 가동 중이며, 재검증은 그 판단이
>   옳았음을 (당시 근거보다 훨씬 강하게) 뒷받침한다.
>
> 로그: `scripts/tier1_recheck.log`. Tier 2 는 여전히 백테스트 불가(macro 표본 5개월).
> **Tier 3 은 `backtest_tier3_alpha.py` 가 yfinance 자체계산이라 이 버그와 무관 — §22.AM 수치는 유효.**
...
### 22.AO-3 변이 평가 인프라 전면 교정 (2026-08-18)

§22.AO-1/2 로 게이트를 열자 이번엔 **변이 평가가 애초에 성립하지 않는 구조**가 드러났다.
배치를 세 번 돌리며 매번 새 결함이 나와, 결국 러너 결함 6종을 찾아 고쳤다.

**결함 ①  baseline 이 라이브 전략이 아니었음**
`_baseline_config` 가 9키만 읽어 ATR 트레일링·부분익절(①번)·Tier1 시장필터·③번 절대모멘텀이
전부 빠진 **'7월 이전 전략'** 을 기준선으로 쓰고 있었다. 변이를 엉뚱한 상대와 비교해 온 셈.
→ `_FIELD_MAP` 9키 → 17키, `_FLAG_MAP` 신설(불리언), 라이브 config 전체를 baseline 에 적재.

**결함 ②  자본 $1,000 하드코딩 (라이브 $2,000)**
`qty = int(금액/주가)` 정수 절삭 때문에 자본이 작을수록 저가주만 담기는 편향 발생.
`position_pct` 를 낮추는 변이는 표본이 잘려 기각(v37: 16거래), 올리는 변이는 현금 고갈로
`max_positions` 가 무의미해짐(v38: Δ0.0). → `_live_capital()` 신설(initial_capital + 입출금 합계).

**결함 ③  고정 손절 폴백 제거 (작업 중 자초한 회귀)**
하드스탑과 중복이라 판단해 `use_hard_stop` 시 고정 `stop_loss_pct` 를 삭제했으나, 라이브
`exit_manager` 는 지우지 않고 **L1 뒤 최종 폴백으로 유지**하는 구조였다. 삭제하자 `stop_loss`
청산이 0건이 되고 v35/41/43 이 전부 Δ0.0. → 폴백 복원, `stop_loss` 청산 24.2% 회복.

**결함 ④  🔴 유령 손실 — 파급 최대**
일말 자산 평가가 `if sym in indicators and day in indicators[sym].index` 로 감싸져 있어,
**해당 일자에 봉이 없는 종목(휴장/결측/상장폐지)의 평가액이 0 으로 사라졌다.** 최종일에 걸리면
`final_value` 가 그대로 깎여 수익률·CAGR·MDD·Sharpe 가 전부 오염.
- 발견 단서: 실현손익 **+$416** 인데 자본증감 **−$170**, 차액 −$587 이 미청산 6건(원가 ~$552)에서
  나올 수 없는 크기였다.
- 수정(마지막 확인가 이월) 효과: **365d −8.52% → +21.57%, Sharpe −0.19 → +2.77**,
  미청산 평가손익 −$586.81 → +$14.98 로 회계 정합.
- ⚠️ **이 러너로 돌린 모든 과거 백테스트에 적용된다** — §22.AL Tier1 검증 재실행 필요
  (§22.AM Tier3 는 `backtest_tier3_alpha.py` 가 yfinance 자체계산이라 **무관**).

**결함 ⑤~⑥  측정 불가 키 3종 (구조적, 미해결)**
| 키 | 원인 |
|---|---|
| `atr_trailing_multiplier` | 트레일링 청산 0건 — `time_stop` 이 선점 |
| `rsi2_exit_threshold` | `rsi2_exit_min_r=2.0` 게이트가 병목 (임계 60/70/80/95 전부 동일 결과) |
| `take_profit_pct` | `use_atr_trailing=True` 면 고정 TP 미검사 (7/27 ①번의 의도된 설계) |

`min_r` 실험으로 병목을 확정: **min_r 2.0 → rsi2청산 19건 / 1.0 → 51건 / 0.5 → 137건.**
임계값은 무관하고 `min_r` 이 유일한 통제 변수였다. 7/1 교정(min_r 1.0→2.0 + thr 90→95)의
효과는 **전적으로 min_r 쪽**이었음. 성과도 현행 2.0 이 최선(Sharpe −0.19 vs 1.0 −0.25, 0.5 −0.22).

**러너에 추가한 5-Layer 정합 (라이브 exit_manager 이식)**
L2 하드스탑 / L4 RSI(2)+min_r 게이팅 / L3 시간청산 을 라이브와 동일 우선순위로 구현하고,
트레일링 SL **래칫**(올리기만) + **브레이크이븐 스톱**(+1R → 본전+0.2%)까지 추가.
RSI(2) 는 라이브 `calc_rsi` 와 같은 **단순평균 방식**(Wilder 아님, avg_loss=0 → 100 고정).

**남은 한계 (해결 못 함)**
- **진입 선별 불일치**: 라이브는 6팩터 composite_score≥61, 러너는 추세+돌파+거래량 규칙.
  포지션 집단이 달라 청산 사유 분포가 안 맞는다(라이브 평균보유 5.3일 vs 러너 15.7일).
  → **절대 수익률은 라이브 예측치가 아니며, 동일 규칙 하 상대 비교로만 유효.**
- **생존편향**: 현재 유니버스를 과거에 적용(2022 백테스트에 SNDK/GEV 등 미존재 종목).
- 멀티팩터 미이식이라 `composite_score_min`·`factor_weight_*` 변이는 영구 측정 불가(4건).

### 22.AO-4 배치 결과 — 이전 판정 전량 폐기

동일 변이를 baseline 을 고쳐가며 4회 평가한 결과, **매번 판정이 달라졌다.**

| variant 17 기준선 | SQN Δ | Sharpe Δ | 판정 |
|---|---|---|---|
| 9키 (7월 이전 전략) | +0.881 | +0.978 | validated |
| 17키 (Tier1+트레일링) | +0.030 | +0.128 | rejected |
| 5-Layer (자본$1k, 유령손실) | −0.298 | −0.242 | rejected |
| 버그수정 (자본$2k) | +0.216 | +0.145 | rejected |

variant 18 은 최초 SQN +1.37/Sharpe +0.99 로 "하네스 사상 첫 합격" 이었으나 최종 +0.122/−0.004
(노이즈 수준). **유령손실 버그가 델타를 양방향으로 무작위 왜곡**하고 있었으므로 이전 배치 결과는
전량 폐기했다.

최종 배치(14건, 58.7분): **validated 5 / rejected 9**. v42≡v46 이라 실질 4가지.
필터 개선으로 대상이 23 → 14건으로 줄고 시간도 96분 → 58.7분.
(no-op 1건 + 무력키만 13건 + 유효키 중복 2건 제외)

### 22.AO-5 🔴 하락장 검증 — 통과 변이 전멸, 분산 정책 유지 확정

365d(상승장)에서 통과한 집중 변이를 하락 구간에 넣자 **결론이 뒤집혔다**(`scripts/bear_market_check.py`).

**2022 약세장** (baseline +0.77%, MDD −3.42%)
| 변이 | 수익률 | MDD |
|---|---|---|
| v30 `position_pct 0.20` | **−3.72%** | **−9.41%** |
| v36 `0.16 / 8개` | −1.90% | −8.73% |
| v22 `0.08 / 5개` | −1.87% | −3.94% |
| v42·46 `0.04 / 10개` | **+0.26%** | **−2.50%** |

**2020 코로나** (baseline −3.83%, MDD −9.66%): v30 MDD **−17.33%**, v36 −16.76%.

- **집중 변이는 상승장 전용.** 하락장에선 수익·낙폭 모두 baseline 열세.
- 통과 원인은 **게이트가 MDD 를 안 봤기 때문** — SQN/Sharpe 는 낙폭에 무감각하다.
- ✅ **2026-07-28 「20개 × 5% 분산」 정책 유지 확정.** 바꿀 근거 없음.
- 유일한 예외: **v42/46(`position_pct 0.04` = 분산 강화 방향)** 이 365d MDD **2.15%**(baseline
  5.09%의 절반) + 2022 약세장 MDD −2.50% 로 양 국면 모두 우수. 별도 검토 가치 있음.

**조치: 낙폭 게이트 신설** — `harness_mdd_tolerance`(기본 0.5). 변이 |MDD| 가 baseline 의
(1+tol) 배를 넘으면 기각. 검산 결과 v30(17.46%)·v36(14.92%) 신규 기각, v22(5.54%)·v42/46(2.15%) 통과.

### 22.AO-6 v42 검토 — '노출 축소'와 '슬롯 집중' 분리 실험

낙폭 게이트 도입 후 유일하게 양 국면 모두 우수했던 v42(`position_pct 0.04 / max_positions 10`)를
배포 후보로 검토. 명목 노출 40%(baseline 100%)라 MDD 가 낮은 것은 당연하므로, **같은 노출을
다른 슬롯 수로 나눠** 무엇이 진짜 변수인지 분리했다(`scripts/exposure_vs_slots.py`).

| 구성 | 명목노출 | **포지션당** | 상승장 수익 | 상승 MDD | 약세장 수익 | 약세 MDD | 거래(상승) |
|---|---|---|---|---|---|---|---|
| C `0.02×20` | 40% | $40 | +2.53% | −0.45% | +0.10% | −0.82% | 44 |
| B `0.04×10` (=v42) | 40% | $80 | +14.31% | −2.15% | +0.26% | −2.50% | 164 |
| E `0.04×20` | **80%** | $80 | +14.44% | −2.04% | +0.20% | −3.42% | 171 |
| **A baseline `0.05×20`** | 100% | $100 | +21.57% | −5.09% | **+0.77%** | −3.42% | 264 |
| D `0.10×4` | 40% | $200 | **+33.15%** | −5.29% | −0.59% | −2.96% | 136 |

**결론 ①  명목 노출은 무의미하고 포지션당 금액이 성과를 지배한다.**
B(40%)와 E(80%)는 노출이 2배 차이인데 결과가 사실상 동일(+14.31% vs +14.44%, MDD −2.15% vs −2.04%).
포지션당 금액이 $80 으로 같기 때문. **`max_positions` 는 이 자본에서 거의 작동하지 않는 파라미터**
(variant 38 `max_positions` 증가가 Δ0.0 이었던 이유와 동일).

**결론 ②  원인은 정수 절삭.** 자본 $2,000·유니버스 주가 $10~250 에서 포지션당 $40(C)이면
`qty = int(40/주가) = 0` 이 빈발해 진입 자체가 무산 → 거래 44건. **C 의 MDD −0.45% 는 방어력이 아니라
'투자를 못 했다'는 뜻.**

**결론 ③  v42 기각.** 우수해 보인 것은 전략적 우위가 아니라 **소액 계좌에서 정수 절삭을 덜 겪는 사이징**의
부산물이며 자본이 커지면 소멸한다.

**부수 발견 — D(`0.10×4`, 포지션당 $200)는 성격이 다르다.**
상승장 **+33.15%**(baseline +21.57%)로 크게 앞서면서 MDD 는 −5.29% vs −5.09% 로 동등, 약세장 MDD 는
**오히려 낮다**(−2.96% vs −3.42%). 기각된 v30/v36(약세장 MDD 3배 + 수익 열세)과 명확히 다르다.
단 **약세장 수익이 −0.59% 로 마이너스 전환**, 4종목 집중은 개별종목 사고에 취약, 2022 단일 표본.
→ **지금 정책을 바꿀 근거로는 부족하나 폐기할 발견도 아님. 보류.**

**실전 함의 (중요):** 현재 `position_pct 0.05 / max_positions 20` = 포지션당 $100 인데, 고가주는 1주도
못 사거나 1~2주에 그쳐 **의도한 20종목 분산이 실현되지 않는다.** 2026-07-28 결정이 틀린 게 아니라,
**소액 계좌에서는 분산 자체가 정수 절삭에 막힌다**는 제약을 당시 반영하지 못했다. 자본 증가 또는
분수주(fractional shares) 도입 시 재검토 대상.

### 22.AO-7 멀티팩터 이식 실현가능성 판정 — 원안 불가

변이 4건(26/32/39/45, `composite_score_min`·`factor_weight_*`)을 측정하려면 러너에 6팩터 스코어링이
필요하나, **팩터별 과거 데이터 가용성 조사 결과 원안은 성립하지 않는다.**

| 팩터 | 가중 | 출처 | 백테스트 |
|---|---|---|---|
| technical | 25% | 시그널 필드(rank/trend/breakout/volume) + LSTM | ✅ 가능 (러너가 이미 계산) |
| macro | 10% | VIX·금리·Cu/Au·DXY·BTC | ✅ 가능 (과거 시세 존재) |
| sentiment | 18% | Finnhub 뉴스 + Claude/Ollama | ❌ 과거 뉴스 스냅샷 없음 |
| flow | 10% | 내부자·추천·공매도잔고 | ❌ 시점별 데이터 없음 |
| quality | 19% | Finnhub 재무 | ❌ **현재 재무를 과거 적용 = 미래참조** |
| value | 18% | Finnhub 재무 | ❌ 동일 |

**백테스트 가능한 것은 35%뿐이고, 나머지 65%는 구현하면 오히려 해롭다** — quality/value 에 현재
재무제표를 쓰면 look-ahead bias 로 실제보다 좋은 결과가 나오는, 유령손실 버그보다 나쁜 오염이 된다.

**선택지:** (A) technical+macro 만 이식(35%) — 라이브와 다른 지표라 정합 안 됨 /
(B) **시그널 리플레이** — `swing_signals` 에 당시 계산된 `composite_score`·팩터별 점수가 보존돼 있어
**미래참조 없이** 검증 가능 (2026-03~08, 267건 중 139건 스코어 보유) /
(C) 시점별 재무 데이터 유료 구매(Sharadar 등).

→ **B 채택.** 표본이 작아 변이 A/B 에는 부족하지만, "composite_score 가 실제로 수익을 예측하는가",
"임계값 61 이 최적인가", "팩터별 기여도" 는 실데이터로 답할 수 있다.

### 22.AO-8 시그널 리플레이 — 팩터 예측력 실측 (B안 실행)

`swing_signals` 에 **당시 계산돼 저장된** 팩터 점수와, 시그널 이후 `daily_prices` 실제 종가로 계산한
전방수익을 대조(`scripts/signal_replay_ic.py`). 저장값만 쓰므로 **미래참조 없음.**

**팩터별 순위상관 IC**
| 팩터 | 가중 | 5일 | 10일 | 20일 |
|---|---|---|---|---|
| **technical** | 25% | +0.167 | **+0.354** | **+0.366** |
| composite(종합) | — | +0.107 | +0.200 | +0.185 |
| macro | 10% | +0.091 | +0.198 | +0.127 |
| quality | 19% | +0.061 | +0.106 | +0.093 |
| sentiment | 18% | +0.020 | −0.034 | +0.005 |
| **value** | **18%** | **−0.177** | **−0.271** | **−0.341** |
| **flow** | **10%** | **−0.273** | **−0.300** | **−0.406** |
표본: 5일 128건 / 10일 102건 / 20일 61건.

- 🔴 **`flow`(10%) + `value`(18%) = 가중 28% 가 수익을 역방향으로 예측**하며, 기간이 길수록 악화.
- 🔴 **종합점수(+0.20)가 technical 단독(+0.35)보다 예측력이 낮다** — 음의 IC 팩터가 신호를 희석.
- `sentiment`(18%)는 IC ≈ 0 으로 사실상 무의미.
- 부호와 크기가 세 기간에서 일관되므로 방향성은 신뢰할 만함.

**composite_score 구간별 10일 수익**
| 구간 | n | 평균 | 승률 |
|---|---|---|---|
| 55–61 | 21 | −0.13% | 42.9% |
| 61–65 | 19 | +6.20% | 73.7% |
...
- 현행 `composite_score_min=61` 은 **방향은 맞다**(55–61 이 확실히 나쁨) **but 최적점은 아니다.**
- **70 이상에서 오히려 성과가 꺾인다**(+10.75% → +4.46%) — 과열 구간. 상한 개념(`auto_approve_score_max=85`)을
  70 근처로 낮추는 것을 검토할 근거.

**한계:** 20일 표본 61건, 세부 버킷은 3~17건이라 임계값 최적점은 표본 축적 필요.
팩터 가중치는 백테스트로 측정 불가 영역(§22.AO-7)이라, 조정 시 **시계열 분할 검증 + paper A/B** 필요.

### 22.AO-9 백테스트↔라이브 격차 분해 — MDD 5배는 '다른 정책을 비교한 것'

사용자 지적: *"시장에서 −를 얻은 뒤 사후 대처로 바꿔서 어떻게 연 18% 를 달성하나"*.
답하려면 먼저 **백테스트가 라이브를 재현하는가**를 확인해야 해서 동일 구간·동일 자본으로 비교했다.

| | 백테스트 | 실제 라이브(paper) |
|---|---|---|
| 기간 | 2026-03-09 ~ 08-18 | 동일 |
| 수익률 | **+3.98%** | **−5.26%** |
| 거래수 | 77건 | 74건 |
| 승률 | 40.3% | 36.7% |
| payoff | 2.25 | 2.45 |
...
⚠️ 이전에 "27%p 격차"라고 적은 것은 **365일 백테스트와 5.5개월 라이브를 비교한 오류**였다. 실제 9.2%p.

**거래수·승률·payoff 는 거의 같은데 MDD 만 5배** → 개별 거래의 질이 아니라 **자금관리 차이**.

**원인: 라이브가 백테스트와 다른 정책으로 돌고 있었다.**
| 월 | 동시보유 평균 | 진입금액 평균 | 자본 대비 |
|---|---|---|---|
| 2026-03 | 3.3개 | $142 | **14.2%** |
| 2026-04 | **1.0개** | $110 | **11.0%** |
| 2026-05 | 1.6개 | $135 | **13.5%** |
| 2026-06 | 2.2개 | $113 | **11.3%** |
| 2026-07 | 3.5개 | $123 | 12.3% |
| 2026-08 | **11.7개** | $104 | **5.2%** |

- 3~6월은 **자본의 11~14% 를 한 종목에** 넣고 평균 1~3종목만 보유. 최저점(6/26 −10.39%)에는
  **오픈 포지션이 1개**뿐이었다.
- 백테스트는 **현재 config(5%×20종목)를 전 기간에 소급 적용** → 당연히 낙폭이 작다.
- **8월에 와서야**(11.7종목·5.2%) 라이브가 백테스트와 같은 정책이 됐다.

**함의**
1. **라이브 −5.26% 의 대부분은 폐기된 집중 정책의 성적**이다. 현재 전략의 실적으로 읽으면 안 된다.
   동일 정책 관측치는 2026-08 이후 3주치뿐 — 판단하기 이르다.
2. 7/28 분산 전환은 이미 작동 중: 6/26 −10.39% 이후 낙폭이 −5%대에서 안정.
3. 백테스트↔라이브 비교는 **2026-08 이후 구간으로만** 해야 유효하다.

**18% 경로 재정리 (냉정한 평가)**
| 단계 | 기여 | 상태 |
|---|---|---|
| 집중 → 분산 전환 | MDD −15% → −3%대 | ✅ 7/28 완료 |
| 계측 오류 제거(유령손실 등) | 판단 신뢰성 | ✅ 2026-08-18 완료 |
| 팩터 가중치 교정 | 검증 IC +0.291 → +0.356 | ✅ 적용, 관찰 필요 |
| **기대수익 8.9% → 18%** | **+9%p** | ❌ **새 엣지 필요** |

백테스트 5.5개월 +3.98% = **연율 약 8.9%** 로, 백테스트가 완벽히 맞아떨어져도 18% 의 절반이다.
앞의 3단계는 **손실을 막는 일**이었고 마지막이 **수익을 만드는 일**이다. 파라미터 튜닝으로 메울
격차가 아니며, 여기서부터는 새로운 알파 탐색이 필요하다.

**생존편향 측정(3번, 부분):** 현재 활성 유니버스 200종목 중 **196종목(98%)이 2022년에도 존재** →
'그때 없던 종목을 넣은' 오염은 4종목(SNDK/GEV/HONA/CEG)으로 미미. 다만 **2022년에 지수에 있다가
이후 탈락한 기업**은 유니버스에 아예 없어 복원 불가(Wikipedia 변경이력 표 접근 실패, 유료 데이터 필요).

### 22.AO-10 새 엣지 리서치 (영문웹 + 최신논문, 2026-08-19)

기대수익 8.9% → 18% 격차(+9%p)를 메울 **새 알파** 탐색. 파라미터 튜닝으로는 닿지 않는 구간.

**⚠️ 기준선 — 교과서 팩터 추가는 기대수익 0**
Chen & Welch, *What Useful Alphas?* (arXiv 2607.06502): published anomaly 약 200개 재검증.
| 조건 | 월 수익 중앙값 |
|---|---|
| ~2005 | 48bp |
| 2005~ | 19bp |
| **2005~ + 비-초소형주** | **7bp** |
결론: *"약간의 운이나 거래비용만 감안해도 이 7bp 조차 사라진다"* → **우리 유니버스(S&P500 대형주)에
교과서 팩터를 더 넣는 접근은 무의미.** 오늘 측정한 flow/value 음수 IC 와 정합.
현실 기준: 리테일 스윙 평균 비용후 **연 +2.1%**, **18%는 상위 10% 영역**.

**① LLM 모멘텀 조건화 — 채택 1순위**
*ChatGPT in Systematic Investing* (arXiv 2510.26228). LLM 에게 **감성을 묻지 않고**
"최근 뉴스가 **과거 수익의 지속**을 지지하는가"를 물어 종목선택+비중을 동시 조건화.
- 학습 컷오프 **이후** 진짜 OOS 에서도 표준 모멘텀 대비 Sharpe·Sortino 우위
- **집중·고확신 포트폴리오에서 효과 최강**, 거래비용·프롬프트설계·제약에 robust
- 우리 `sentiment_score` IC ≈ 0(5일 +0.020 / 10일 −0.034)인 이유 설명: LLM 이 무용한 게 아니라
  **질문이 틀렸다**. 별도 연구도 naive LLM 질의는 월 0.35%로 0과 구분 불가.
- Ollama + Finnhub 뉴스 + 모멘텀 진입이 이미 있어 **프롬프트 재설계만으로 전환 가능**

**② 신호 교집합 (Intersection) — 채택 2순위**
Sobotka, *The Intersection of Expected Returns* (2025), anomaly 164개.
- 여러 anomaly 에 **동시 등장하는 상위 10% 종목**이 전체 anomaly 수익의 **약 40%** 차지
- 교집합 종목 알파가 나머지보다 **3배 이상**, 164개 anomaly 전반에 균일
- **오늘 발견을 설명**: composite(IC +0.20) < technical 단독(+0.35) — **가중평균이 신호를 희석**.
  교집합은 일치할 때만 진입해 신호를 농축 → `composite_score ≥ 61` 을
  **"독립 신호 N개 이상 동시 충족"** 으로 구조 변경

**③ PEAD 부활 (12분기 이력 + ML) — 채택 3순위**
Kaczmarek & Zaremba, *Beyond the last surprise*, Finance Research Letters 2025-10.
- SUE 를 1분기가 아닌 **12분기 이력**으로 쓰면 **Sharpe 거의 2배**
- **대형주에서 최강** (최근 서프라이즈는 즉시 반영되나 과거 패턴은 간과)
- 드리프트는 애널리스트 상향이 **2~4주** 순차 반영되며 발생 → **스윙 5~15일과 정확히 일치**
- watchlist 5-Layer 의 Layer 3 에 PEAD 가 **placeholder 로 비어 있음**, Finnhub 어닝스 이미 수신 중

**④ 가격-거래량 마이크로구조 17종 — 채택 4순위**
Du·Walter·Ulrich, *Cross-Market Alpha* (arXiv 2601.06499). 단기신호 191개를 S&P500(2002~2022)에서
double-selection LASSO 로 검증, 펀더멘털 151개 통제 후에도 **독립 유의 17개**. 전부 가격·거래량 기반이라
무료 데이터로 계산 가능. 월간 리밸런싱에서도 희석되지 않음.

**❌ 제외 결정**
- **옵션 신호(IV skew/put-call spread)**: 2025 JFE 연구 — 예측력이 대부분 **공매도 차입수수료 대리변수**이며
  고수수료 종목 제외 시 **최소 2/3 감소**. S&P500 대형주는 차입수수료가 낮아 **우리 유니버스에선 무의미**
- **야간 드리프트**: 실재하나(SPY 5년 CO +47.1% vs OC +29.9%) 멀티데이 보유라 **이미 수취 중**.
  진입 체결 타이밍에만 소폭 활용 가치

**구축 순서 (사용자 승인)**: ① → ② → ③ → ④.
①② 는 새 데이터 불필요(프롬프트·스코어링 수준)이라 선행. 오늘 확인한 집중 사이징(D: 4종목×$200 → +33%)과
...
### 22.AO-12 ② 신호 교집합 — 게이트로 채택 (순위지표로는 기각)

Sobotka(2025) 교집합 알파 3배 + 우리 실측(composite +0.20 < technical 단독 +0.35 = 가중평균의 희석)에서
출발. `swing_signals` 에 팩터 점수가 보존돼 있어 **①과 달리 즉시 측정 가능**했다.
시계열 분할(학습 2026-03~05 / 검증 06~08, 전방 10일, 103건)로 검증(`scripts/intersection_study.py`).

**순위지표(IC)로는 기각 — 교집합이 technical 단독을 못 이긴다**
| 지표 | 학습 IC | 검증 IC | 최솟값 |
|---|---|---|---|
| composite (현행 가중평균) | +0.117 | +0.258 | 0.117 |
| **technical 단독** | **+0.279** | +0.334 | **0.279** |
| 교집합 6화면 | +0.002 | +0.467 | 0.002 |
| 교집합 3화면 | +0.146 | +0.454 | 0.146 |
| 교집합 2화면 | +0.144 | +0.476 | 0.144 |
| momentum rank 단독 | +0.353 | +0.253 | 0.253 |
교집합은 **검증에서만 높고 학습에서 낮아 불안정**. 개수(0~6)는 값이 6개뿐이라 순위상관에 불리한 것도 원인.

**게이트로는 채택 — 현행 대비 전 지표 우위**
| 게이트 | 통과 | 평균수익 | 승률 | 학습 | 검증 |
|---|---|---|---|---|---|
| 없음 | 103 | +4.59% | 57.3% | +9.30% | −0.61% |
| 현행 composite≥61 | 65 | +6.55% | 66.2% | +10.76% | +1.64% |
| 교집합≥4 (6화면) | 84 | +6.07% | 61.9% | +10.49% | +0.71% |
| **momentum + technical 동시** | **55** | **+7.90%** | **76.4%** | +9.96% | **+3.64%** |

- **승률 66.2% → 76.4%**, 검증구간 평균수익 **+1.64% → +3.64%(2.2배)**, 중앙값 +3.01% → +4.20%
- **화면을 더 얹으면 오히려 나빠진다**(교집합≥4 는 +6.07%). Sobotka 의 교집합 개념은 맞지만
  **우리 팩터에서는 2개면 충분하고 그 이상은 희석.**
- `trend` 는 103건 중 96건 충족이라 **구속력이 없어 무의미**(momentum+technical 과 결과 동일)

**화면별 단독 기여 (10일 평균수익 차이)**
momentum **+8.42%p** · technical +4.68%p · trend +3.73%p · quality +0.86%p · macro +0.23%p ·
**breakout −0.88%p(음수)**. 브레이크아웃+거래량 확인은 오히려 해가 된다.

**적용**: `auto_approve.py` PASS 1 에 교집합 게이트 추가.
config `intersection_gate_enabled`(true) / `intersection_momentum_min`(0.70) / `intersection_technical_min`(60).
되돌리기: `intersection_gate_enabled=false`.
검증 시뮬레이션: WFC(rank 0.43)·CVX(0.66)·OXY(0.66) 는 현행 게이트를 통과하나 교집합에서 차단됨.

⚠️ **한계**: 승리 게이트의 검증 표본 n=18. 통과 건수가 65→55 로 줄어 거래빈도 감소.
표본 축적 후 재확인 필요.

### 22.AO-13 breakout 음수 기여 진단 — 보유기간에 따라 부호가 뒤집힘

§22.AO-12 에서 breakout 만 유일하게 음수 기여(−0.88%p)로 나와 분리 진단(`scripts/breakout_check.py`).

**시그널 실측 — 부호가 horizon 에 따라 반전**
| 전방 | breakout_5d 충족 vs 미충족 |
|---|---|
| 5일 | +2.22% vs +5.98% → **−3.76%p** |
| 10일 | +4.62% vs +7.76% → **−3.14%p** |
| 20일 | +10.71% vs +8.40% → **+2.31%p** |

5일 고점 돌파 진입 = **단기 과열 매수** → 1~2주 되돌림 후 20일쯤 모멘텀 재작동.
**우리 평균 보유는 라이브 5.3일 / 백테스트 11.7일**로 정확히 해로운 구간에 있다.
승률은 오히려 높다(64% vs 58%) — **작은 이익을 자주 내고 큰 상승을 놓치는** 구조.

**백테스트 확증 (require_breakout 은 러너 파라미터라 검증 가능)**
| 구간 | 설정 | 수익률 | Sharpe | MDD | 거래 | 승률 | payoff |
|---|---|---|---|---|---|---|---|
| 365d 상승장 | True(현행) | +21.41% | 2.75 | −5.09% | 262 | 50.8% | 1.68 |
| | **False** | **+29.67%** | **2.86** | **−4.38%** | 369 | 48.2% | **1.85** |
| 2022 약세장 | True(현행) | +0.78% | 0.21 | **−3.42%** | 89 | 47.2% | 1.27 |
| | False | +0.72% | 0.16 | −4.73% | 130 | 43.9% | 1.38 |

- 상승장: 해제 시 **수익 +8.3%p, Sharpe·MDD·payoff 모두 개선.** 승률은 하락(50.8→48.2%)하나
  payoff 상승(1.68→1.85)이 상쇄 — 시그널 분석의 "작은 이익 자주, 큰 상승 놓침"과 정확히 일치
- 약세장: 수익 동일하나 **MDD −3.42% → −4.73% 악화** (진입 89→130건으로 하락장 노출 증가)

**결론: 단순 해제는 부적절.** 상승 국면에서만 완화하는 **레짐 조건부**가 정합적이나, 이는 새 로직이라
...
### 22.AO-15 관찰 체계 — 주간 팩터 IC 자동 측정

①LLM 모멘텀(A/B 로깅) · ③PEAD 는 **백테스트로 측정 불가**해 표본 축적으로만 검증된다.
수동 스크립트에 의존하면 사후 대처가 반복되므로 **자동 측정·이력 축적·리포트**를 만들었다.

**구성** (`engine_v4/analysis/factor_ic.py`)
- 미래참조 없음: `swing_signals` 에 **당시 저장된** 점수만 쓰고 수익은 이후 실제 종가로 계산
- 5/10/20일 전방수익 × 9개 팩터(신규 `llm_momentum_score` 포함) 순위상관 IC
- **②교집합은 IC 가 아니라 '통과분 성적'으로 측정** — 순위지표로는 부적합하다고 §22.AO-12 에서
  확인했으므로 게이트 성능(통과/미통과 평균수익 차이)으로 본다
- `swing_factor_ic` 테이블에 누적 → IC 추세를 시간축으로 볼 수 있음
- 스케줄러 `factor_ic` 일요일 12:00 KST (pead_collect 11:00 직후) → 총 **27 잡**
- 텔레그램 리포트 발송(`factor_ic_notify`)

**첫 측정 (2026-08-19)**
| 팩터 | 5일 | 10일 | 20일 |
|---|---|---|---|
| momentum(rank) | — | **+0.431** | +0.185 |
| technical | +0.182 | +0.356 | **+0.366** |
| composite | +0.121 | +0.199 | +0.185 |
| macro | +0.045 | +0.173 | +0.127 |
| quality | +0.068 | +0.097 | +0.093 |
| sentiment ①대조 | +0.031 | −0.037 | +0.005 |
| **value** | −0.129 | −0.263 | **−0.341** |
| **flow** | −0.265 | −0.301 | **−0.406** |

**②교집합 게이트 — 세 horizon 모두 우위 확인**
| 전방 | 통과 | 통과 평균 | 승률 | 미통과 | 차이 |
|---|---|---|---|---|---|
| 5일 | 64건 | +4.93% | 69% | +2.86% | **+2.07%p** |
| 10일 | 55건 | +7.90% | 76% | +4.83% | **+3.07%p** |
| 20일 | 38건 | +13.18% | 74% | +8.46% | **+4.72%p** |

`llm_momentum_score` 는 아직 표본 0 이라 미표시(정상) — A/B 로깅이 쌓이면 `sentiment_score` 와
같은 표에서 직접 비교된다.

### 22.AO-16 ④ 마이크로구조 17종 — **기각** (재현 실패)
...
(펀더멘털 151개 통제 후, 프리미엄 25~79bp, 월간 리밸런싱).

논문 본문에서 17개 전체 목록 확보. 상위 프리미엄:
046 다기간 평균회귀(79bp, t=3.68) · 071 24일 평균괴리(58bp) · 084 20일 누적OBV(51bp, t=3.68) ·
063 6일 RSI(45bp) · 054 일중변동성(44bp) · 123 가격거래량 상관순위(42bp) · 015 야간갭(41bp) ·
184 지연 가격갭상관(41bp) · 039 VWAP 괴리(39bp) · 181 초과수익 왜도(38bp) · 001 거래량-수익 상관(37bp) ·
190 로그 상승/하락 변동성비(36bp) · 161 12일 ATR(34bp) · 086 가격가속(32bp) · 049 하락압력비(25bp) ·
073 중첩 감쇠 가격거래량 상관 · 155 거래량 MACD

**우리 데이터로 직접 재현 검증** (`scripts/microstructure_study.py`)
- 유니버스 200종목 × 2016~2026, 일간 OHLCV **50.5만행** → 주간 횡단면 10.3만행
- 시계열 분할: 학습 2016~2021(309일) / 검증 2022~2026(238일)
- 일간 OHLCV 로 구현 가능한 10종 근사 구현 (VWAP·중첩상관 필요분 제외)
- 지표: 일자별 횡단면 순위상관의 평균(표준 IC)

**결과 — 10종 중 9종이 out-of-sample 실패**
| 신호 | 5일 최솟값 | 10일 | 20일 |
|---|---|---|---|
| **atr12 (161)** | **+0.025** | **+0.033** | **+0.045** |
| gap (015) | −0.003 | +0.008 | +0.006 |
| vmacd (155) | −0.000 | +0.005 | +0.004 |
| mr_multi (046) | +0.003 | −0.005 | −0.015 |
| dev24 (071) | −0.002 | −0.009 | −0.020 |
| obv20 (084) | −0.006 | −0.009 | −0.016 |
| rsi6 (063) | −0.016 | −0.013 | −0.018 |

**논문이 최고 프리미엄으로 꼽은 평균회귀(046 79bp, 071 58bp)가 학습에서만 양수, 검증에서 음수.**
OBV(084)·RSI6(063)도 동일. → **Chen & Welch(2026) 의 "2005년 이후 대형주에서 published anomaly 는
무용"(§22.AO-10) 경고가 우리 데이터에서 그대로 재현됨.**

**유일 생존 atr12 도 알파가 아님** (`scripts/atr_regime_check.py`)
| 국면 | atr12 → 전방10일 IC |
|---|---|
| 상승국면 (SPY>SMA200) | +0.0222 (455일) |
| 하락국면 전체 | +0.0971 (88일) |
| **2022 약세장** | **−0.0548** (52일) |
| 전체 | +0.0343 (543일) |
하락국면 전체가 강양수인 것은 **2020 코로나 급반등**(고베타 폭등)이 포함된 탓이고, 완만한 하락장인
**2022 년에는 부호가 뒤집힌다.** → 안정적 알파가 아니라 **국면 의존적 고베타 노출**.

**결론: ④ 전면 기각. 구현하지 않는다.** IC 크기도 0.02~0.05 로, 우리가 이미 쓰는 momentum(+0.43)·
technical(+0.36) 대비 미미하다. 논문 수치를 그대로 믿지 않고 재현 검증한 것이 옳았다.

### 22.AO-17 ①③ 자동 검증 판정 — 알림 루프 완성

관찰 체계(§22.AO-15)를 만들었으나 두 구멍이 있었다.
1. **③PEAD 점수가 시그널에 저장되지 않았다** — 워치리스트 분석에만 있어 IC 측정 대상 밖
2. **리포트가 IC 표만 보여줬다** — "검증됨/미달"을 사람이 매주 해석해야 함 → 결국 놓친다

**수정**
- `swing_signals` 에 `pead_score`, `pead_signal` 추가. `MultiFactorScorer.score_signal` 이
  매 스코어링마다 PEAD 를 계산해 **composite 반영 없이 기록**(①과 동일한 관찰 방식)
- `factor_ic.verdicts()` 신설 — 기준을 코드에 박고 판정을 자동화

**검증 기준 (명시)**
| 항목 | 기준 |
|---|---|
| 최소 표본 | n ≥ 30 (미만이면 "측정중") |
| 판정 horizon | 10일 (IC 신호가 가장 뚜렷) |
| ① LLM모멘텀 | IC 가 sentiment 대비 **+0.05 이상** 우위 + 자체 양수 + 3개 horizon 중 2개 이상 양수 |
| ③ PEAD | IC **≥ +0.05** + 3개 horizon 중 2개 이상 양수 |

**리포트 구조**: 판정을 **맨 앞**에 배치. 기준 충족 시 `🔔 기준 충족 — 적용 검토 필요` 와
구체적 적용 방법(`llm_momentum_active=true` / composite 에 pead 편입)을 함께 표시.
일요일 12:00 KST 텔레그램 자동 발송.
...
### 22.AO-19 3I 팩터 가중치 자동 튜닝 — 하네스의 구조적 사각지대 제거

**사용자 지적**: *"지난 2~3일의 과정을 Autonomous Evolution Harness 가 스스로 했어야 하는데
그러지 못해 실망스럽다. 논문을 백테스트하고 주간 단위로 시장·데이터를 분석해 계속 업데이트하게 하라."*

**하네스가 못 했던 이유 — 능력 부족이 아니라 구조적 차단이었다**
| | 하네스 | 2026-08-17~19 수동 작업 |
|---|---|---|
| 논문 수집 | ✅ arxiv/Reddit **초록을 텍스트로** 저장 | 본문에서 17개 신호 추출 |
| 변이 생성 | ✅ **config 값만** 변형 | 새 신호 구현 |
| 백테스트 | ✅ (오늘 교정) | 시계열 분할 검증 |
| **팩터 IC 측정** | ❌ 없음 | ✅ flow/value 음수 발견 |
| **가중치 조정** | ❌ **불가능** | ✅ 교정 |

- `TUNABLE_PARAMS` 에 `factor_weight_*` 가 **아예 없었고**
- `REGIME_WEIGHTS` 는 **코드 하드코딩**이라 하네스가 손댈 방법이 없었다
→ "flow/value 가 음의 IC 인데 17% 가중" 문제를 **영원히 발견할 수 없는 구조**였다.

**조치 1 — 가중치를 코드에서 DB 로 이관**
`swing_factor_weights`(레짐×팩터) + `swing_weight_history`(변경 이력) 신설.
`MultiFactorScorer._load_weights()` 가 DB 를 우선 읽고 없으면 하드코딩 폴백.
**이제 가중치가 하네스의 조정 대상이 된다.**

**조치 2 — 3I `weight_tuner.py` 신설 (사람이 한 판단을 매주 자동 수행)**
- `swing_factor_ic` 에 누적된 최신 IC 를 읽어 **목표 가중치 = max(0, IC) 비례 배분**
- 음수 IC 팩터는 목표 0 → flow/value 같은 사례가 **자동으로 걸러진다**
- 안전장치: 최소 표본(40) · 주당 변화폭 상한(0.05) · 팩터별 하한/상한 · 합계 1.0 정규화 ·
  전 변경을 사유와 함께 `swing_weight_history` 기록(되돌리기 가능)
- `BOUNDS`: technical 0.20~0.50(실측 최강이라 하한 높임), flow/value 0~0.15, 나머지 0~0.30

**조치 3 — 주간 사이클에 편입**
`factor_ic`(일 12:00 KST) 직후 **같은 잡에서** `weight_tuner` 실행 → 텔레그램 리포트 병합.
*측정만 하고 반영을 사람이 하면 결국 사후 대처가 된다*는 점을 구조로 막았다.

**조치 4 — 변이 탐색 공간 확장**
`TUNABLE_PARAMS` 에 오늘 유효성이 확인된 노브 추가:
`intersection_momentum_min`(0.50~0.85) · `intersection_technical_min`(40~75) ·
`pead_drift_days`(10~45) · `rsi2_exit_min_r`(0.5~3.0)

**dry-run 검증 (2026-08-19)**
측정 IC: technical +0.356 · macro +0.173 · quality +0.097 · **value −0.263 · flow −0.301**
보류: sentiment(표본 0/40) · pead(표본 0/40) ← **표본 없는 팩터는 건드리지 않는다**
제안 변경 20건 (예: HIGH_VOL technical 0.280→0.328, quality 0.250→0.199)

**여전히 하네스가 못 하는 것 (정직히)**
- 논문 본문을 읽고 **새 신호를 코드로 구현**하는 일 (오늘 ④의 17개 신호 구현 같은 작업)
- 자기 계측 버그 발견 (유령손실 같은)
- 백테스트 불가 영역의 판단
→ 이 부분은 여전히 사람+LLM 세션이 필요하다. 다만 **측정→조정 루프는 이제 자동**이다.

### 22.AO-20 현 설정 CAGR 실측 — 18% 목표 재평가

오늘 적용분(팩터 가중치 교정 · ②교집합 게이트 · ①③)을 반영한 설정으로 백테스트.
자본 $2,000, `_baseline_config` 가 라이브 config 전체를 읽는 경로.

| 구간 | 총수익 | **CAGR** | Sharpe | MDD | 거래 | 승률 | payoff |
|---|---|---|---|---|---|---|---|
| 최근 1년 | +21.41% | **+21.51%** | 2.75 | −5.09% | 262 | 50.8% | 1.68 |
| 최근 2년 | +42.36% | **+19.44%** | 2.00 | −7.29% | 646 | 52.2% | 1.43 |
| 최근 3년 | +109.82% | **+28.23%** | 2.21 | −7.92% | 1,274 | 53.4% | 1.46 |
| **2022 약세장** | +0.78% | **+0.78%** | 0.21 | −3.42% | 89 | 47.2% | 1.27 |
| **2022~현재 (4.6년)** | +148.27% | **+21.84%** | 1.94 | −10.54% | 1,681 | 52.2% | 1.51 |

**🔴 §22.AO-9 의 "연율 8.9% 가 상한" 주장을 정정한다.**
그 수치는 라이브 동일구간(5.5개월) 백테스트 +3.98% 를 연율화한 것으로 **구간이 짧아 대표성이 없었다.**
4.6년·1,681거래 기준 **CAGR +21.84%** 로, 약세장을 포함하고도 18% 목표를 넘는다.

**⚠️ 유보 3가지 (이 수치를 달성 근거로 삼기 전에)**
1. **생존편향** — 현재 유니버스 200종목을 과거에 적용. 전방오염은 4종목으로 미미하나(§22.AO-9),
   2022년에 지수에 있다가 **탈락한 부진 기업의 누락은 복원 불가**. 통상 수익률을 과대평가한다.
2. **라이브 격차** — 같은 구간 백테스트 +3.98% vs 라이브 −5.26%(§22.AO-9). 원인은 정책 불일치로
   규명됐고 2026-08 부터 정합됐으나 **동일 정책 관측치는 아직 3주치**뿐이다.
3. **2022 약세장 +0.78%** — 하락장에서 '잃지 않는' 수준이지 버는 수준이 아니다.
   4.6년 CAGR 은 상승장 구간이 끌어올린 결과이며, 하락장이 길어지면 연평균은 내려간다.

**⚠️ 백테스트가 반영하지 못하는 것**: ①LLM 모멘텀 · ②교집합 게이트 · ③PEAD · composite 스코어링은
**시그널 경로에만 있어 러너가 시뮬레이션하지 못한다**(러너는 돌파·추세·거래량 규칙으로 진입).
즉 위 수치는 **사이징·청산·Tier1 필터·절대모멘텀**의 성적이다.

**③ PEAD 유니버스 백필 완료 (2026-08-19)**: 200/200 종목, 797행.
신호 분포 POSITIVE_DRIFT 129 · WEAK 49 · NEGATIVE_DRIFT 20 · NONE 2 → **149/200 이 중립 아님.**
상위 SMCI 92.2(서프 +74%) · LLY/UBER/PSX/OXY 88.3. 하위 COIN 16.7(−574%) · BA 18.3(−142%).
⚠️ EPS 추정치가 0 에 가까우면 서프라이즈%가 −574% 같은 극단값이 된다. ±20% 포화로 점수는
안전하나 그런 종목이 실제 드리프트 대상인지는 별개 문제 — 향후 이상치 필터 검토 대상.

**세션 중 내 보고 오류 2건 (기록)**
1. 팩터 가중치 "적용 완료"(§22.AO-8) → `REGIME_WEIGHTS` 하드코딩에 막혀 **무효**였음(§22.AO-18)
2. "연율 8.9% 가 상한"(§22.AO-9) → 5.5개월 구간 연율화 오류, 4.6년 기준 21.84%
둘 다 **더 넓은 데이터로 확인했을 때 뒤집혔다.** 유령손실 버그·Tier1 재평가·variant 판정 4회 변동과
같은 성격. **수치 보고 시 구간 길이와 적용 경로를 먼저 확인할 것.**

### 22.AO-21 3K 자가진단 — 계측 버그를 시스템이 직접 잡는다

**동기**: 2026-08-18~19 에 발견한 버그 4종은 전부 **불변식 위반**이었다. 사람이 의심해서 찾았지만,
한 번 알아낸 것은 두 번 다시 놓치지 않아야 한다.

**검사 6종** (`engine_v4/harness/self_check.py`)
| 검사 | 심각도 | 잡아내는 버그 |
|---|---|---|
| `snapshot_identity` | critical | cash + invested ≠ total_value |
| `metric_range` | critical | **MDD −505%**, cum_return 이상치 |
| `equity_continuity` | warn | 자산곡선 급변(평가 오류) |
| `weight_consistency` | critical | **REGIME_WEIGHTS 하드코딩**(DB 변경 무시) + 합계 ≠ 1.0 |
| `factor_variance` | warn | **Δ0.0 무력 키**(팩터가 상수면 변이가 결과를 못 바꿈) |
| `backtest_identity` | critical | **유령손실**(실현 + 미청산평가 ≠ 자본증감) |

**🔬 회귀 테스트 — 실제로 잡는지 검증**
정리 전 원본 스냅샷(`scripts/backup/swing_snapshots_20260818.csv`)에 규칙을 적용:
- `max_drawdown = −505.1%` → "MDD < −100%" 위반 **검출 ✅**
- `cumulative_return = +494.7%` → "cum > 300%" 위반 **검출 ✅**
→ 이 검사가 있었다면 **사람이 발견하기 전에 시스템이 먼저 경고**했을 사안이다.

**오탐 제거**: 초기 실행에서 `equity_continuity` 가 2026-07-28 $958→$1,958 을 위반으로 신고했는데
이는 **정상 자본 입금 $1,000** 이었다. 입출금일을 제외하도록 수정.
*늑대 소년이 되면 검사 자체가 무시되므로 오탐 제거가 검사 추가만큼 중요하다.*

**운영**
- 스케줄러 `self_check` 일요일 13:00 KST (pead_collect 11:00 → factor_ic 12:00 → self_check 13:00) → 총 **28 잡**
- **위반이 있을 때만 텔레그램 발송** — 매주 PASS 알림은 소음이 되어 무시된다
- API: `GET /harness/self-check`(이력) · `POST /harness/self-check/run`(즉시 실행)
- 이력은 `swing_self_check` 에 누적

**현재 상태**: 6종 전부 PASS.

⚠️ **한계 (정직히)**: **알려진 불변식만** 검사한다. 유령손실을 찾은 것은 "실현손익과 자본증감이
안 맞는데?"라고 **의심한** 덕분이고, 새로운 종류의 버그는 여전히 사람이 발견해야 한다.
다만 발견된 버그는 즉시 불변식으로 추가해 재발을 막을 수 있다.
...
### 22.AO-23 3J 앞단 — 논문 초록 → 수식 자동 추출 (연결고리 완성)

`weekly_research` 가 모은 논문 초록은 **텍스트로만 쌓였고**, 거기서 신호를 뽑아
`swing_signal_formulas` 에 넣는 단계가 비어 있었다. 그래서 3J(수식 검증)는 늘 pending 0 이었다.

**안전 구조**: LLM 이 무엇을 뱉든 `signal_dsl.validate()` 화이트리스트를 통과해야만 저장된다.
악의적/잘못된 출력은 저장 단계에서 거부되므로 임의 코드 실행 경로가 생기지 않는다.

**⚠️ 소형 모델 한계 — ①(§22.AO-11)과 같은 교훈 반복**
처음엔 `{"formulas":[{...}]}` JSON 다중 출력을 요구했으나 **3B 모델이 설명문만 뱉었다.**
양성 대조(초록에 신호가 **명시된** 경우)에서도 0건 추출.
| 프롬프트 방식 | 결과 |
|---|---|
| A) JSON 다중 출력 | *"To create a formula, we need to define a few terms…"* (설명문) |
| **B) 예시 + 평문 한 줄** | **`-(close / ts_mean(close, 20) - 1)` 정확 산출** ✅ |
| C) 빈칸 채우기 | 설명문 |
→ **예시 기반 단일 출력**으로 재설계. 파싱도 JSON 이 아니라 평문 수식 우선(JSON 은 폴백),
코드펜스 제거, `NONE` 처리, "설명문인지 수식인지" 판별(허용 토큰 포함 여부) 추가.

**파서 회귀 테스트 5종 전부 통과**
평문 수식 / `NONE` / 코드펜스 / JSON 폴백 / 설명문 거부.
...
검증: 10건 → 승격 0 / 기각 10
```
기각 내역(학습/검증): +0.0059/+0.018 · +0.0141/−0.0107 · +0.0156/−0.0061 · +0.0117/−0.0082 …

**기각이 정답이다.** 추출된 수식 상당수는 초록의 모호한 서술에서 모델이 **지어낸 것**이다
(예: MoE 변동성 논문 → `-(high - low) / ts_std(open, 20) - 1`). IC 게이트가 정확히 걸러냈다.
→ **추출은 관대하게, 검증은 엄격하게.** 이 비대칭이 설계의 핵심이며 의도대로 동작한다.

**운영**
- `weekly_research`(토 08:00) **직후** 같은 잡에서 `extract_pending` 실행 → pending 적재
- 3.5시간 뒤 `formula_lab`(토 11:30)이 검증 → 승격/기각
- `swing_formula_extraction` 에 논문별 시도 이력(제안/채택/거부) 기록 — 재시도 방지
- config: `formula_extract_enabled`(true) · `formula_extract_max`(8)
- `swing_signal_formulas` 에 `knowledge_id`·`extracted_by` 컬럼 추가 → 출처 추적

⚠️ **한계**: 초록에는 수식 정의가 드물어 **추출 품질이 낮다**. 본문(arxiv HTML)까지 읽으면
품질이 오르겠지만 미구현. `source_url` 은 184건 전부 arxiv 주소로 보존돼 있어 후속 확장 가능.
현재는 **IC 게이트가 품질을 담보**하는 구조다.

### 22.AO-24 논문 본문 파싱 — 초록의 한계 보완 (구현 완료, 효과는 미입증)

§22.AO-23 에서 초록만으로는 수식 정의가 거의 없어 추출 품질이 낮았다. 본문을 읽도록 확장.

**구현** (`engine_v4/harness/paper_fetch.py`)
- arxiv `abs`/`pdf` URL → **`html` 본문 URL 변환** 후 BeautifulSoup(lxml) 로 평문화
  (PDF 는 압축 스트림이라 2026-08-19 파싱 실패 이력 → **HTML 만** 시도)
- **참고문헌 이후 절단**(`references`/`bibliography`/`acknowledg` 등) — 인용 목록이 신호로 오인되는 것 방지
- **신호 정의 키워드 27종**(`we define`, `is computed as`, `rolling`, `turnover`, `reversal` …)이
  포함된 문단만 점수순 선별 → **4,000자 상한**(3B 모델 컨텍스트에 맞춤)
- `formula_extractor` 가 본문 우선, 실패 시 초록 폴백. `source` 상태를 이력에 기록

**본문 수집 성공률**: 시도 10건 중 **9건 `fulltext`** / 1건 `no_html`. 논문당 3,300자 내외 추출.

**⚠️ 추출 품질 개선은 입증되지 않았다 (정직히)**
동일 논문 6건에 초록/본문을 각각 넣어 비교:
| 입력 | 유효 수식 |
|---|---|
| 초록 | 6건 |
| 본문 | 5건 |
개수는 비슷하고 **내용만 달라진다.** 3B 모델은 입력이 무엇이든 그럴듯한 수식을 지어내는 경향이 있어,
**정보량이 늘어도 정확도가 따라 오르지 않는다.**

**🔬 end-to-end (본문 기반)**
```
추출: 논문 10건 → 채택 6 (fulltext 9 / no_html 1)
검증: 6건 → 승격 0 / 기각 6
```
기각 내역(학습/검증): +0.0033/−0.0065 · +0.0123/+0.0015 · +0.0034/+0.0011 ·
+0.0109/−0.0034 · +0.0029/−0.0039 · **+0.0223/+0.0183**

마지막 건(`-(high - ema(high, 10) / high - 1)`)은 **양쪽 구간 모두 양수**지만 기준 0.02 에 검증이
0.0183 로 미달해 기각됐다. 기준을 조금만 낮췄다면 통과했을 것 — 경계 사례다.
승격 0 은 **게이트가 제 역할을 한 결과**이지 파이프라인 고장이 아니다.

**API 추가**
`POST /harness/formulas/extract`(추출 실행) · `GET /harness/formulas/extraction-log`(논문별 시도 이력)

**종합 평가**
- 본문 파싱은 **기술적으로 완성**됐고 수집 성공률 90%
- 그러나 **추출 정확도의 병목은 입력이 아니라 모델 능력**이다. 3B 로는 논문 서술을 수식으로
  옮기는 작업이 신뢰할 수준이 못 된다. Claude 사용 시(`anthropic_key` 설정) 자동으로 그쪽을 쓰므로
  품질이 크게 오를 여지가 있다 — 현재는 Ollama 폴백.
- **IC 게이트가 품질을 담보**하는 구조는 유지된다. 지어낸 수식이 들어와도 승격되지 않는다.

### 22.AO-25 세션 마무리 — 잔여 정리 및 미결 항목 (2026-08-20)

**사용자 결정**: 유료 전환 없음. **Ollama 무료 경로 유지** (`ai_mode: ollama` 확인).
Claude 경로는 전부 `if anthropic_key and ...` 가드가 있어 키 없이도 전 기능 동작한다.

**🔴 잔여 위험 1건 발견·제거 — validated 변이가 MDD 게이트를 우회할 수 있었다**
낙폭 게이트(§22.AO-21)는 **도입 이후 검증분에만** 적용되는데, 그 이전에 validated 된 5건이
남아 있었다. `deploy_best_variant` 는 `sqn_delta` 최고순으로 고르므로:
| 변이 | sqn_delta | MDD | 판정 |
|---|---|---|---|
| v36 모멘텀 스트레칭 | +1.005 | **14.92%** | 🔴 게이트 거부 대상인데 **배포 1순위**였음 |
| v30 투자포트폴리오 | +0.924 | **17.46%** | 🔴 거부 대상 |
| v22 보조지표 줄임 | +0.184 | 5.54% | OK |
| v42·46 | −0.156 | 2.15% | OK |
→ **MDD 게이트 소급 적용**해 v30·v36 을 rejected 로 전환. 이제 배포 1순위는 **v22**(MDD 5.54%,
baseline 5.09% 수준). *게이트를 새로 만들면 기존 통과분에도 소급 적용해야 한다 — 안 그러면
낡은 판정이 새 안전장치를 우회한다.*

**정리 상태**
- git: `main...origin/main` 동기, 미커밋/미추적 파일 없음
- DB 테스트 잔여물: 없음(양성 대조용 knowledge/수식 삭제 완료)
- `swing_self_check`: PASS 13 / FAIL 1 (FAIL 은 오탐 수정 **전** 기록, 감사 이력으로 보존)

**현재 운영 상태 (2026-08-20)**
오픈 10 / 청산 82, 총자산 $1,917.27, 누적 −4.14% (전일 −5.26% 대비 개선)

**미결 항목 (우선순위순)** — #1·#9 는 2026-09-04 실측으로 갱신(§22.AO-28), 나머지는 2026-08-20 기준
| # | 항목 | 상태 | 비고 |
|---|---|---|---|
| 1 | **①③ 검증** | 표본 **15/30** (2026-09-12 `factor_ic` 실측) | ⚠️ 표본은 `swing_signals` 의 점수 기록 개수(9/4 기준 27, 9/12 44)가 **아니라** `swing_factor_ic.n_samples` 로 센다 — h=10d 선행수익률이 확보된 쌍만 들어가므로 최근 시그널은 아직 못 센다. 2026-09-04 에 "27/30, 3건 남음"이라 적은 것은 **잘못된 지표를 본 것**이다.<br>**첫 측정(2026-09-12, h=10d): ③PEAD IC +0.5833 / ①LLM모멘텀 IC −0.4251, 둘 다 n=15.** 기준(n≥30) 미달이라 판정은 아직 "측정중"이지만, ①이 강한 음수인 점은 주시할 것 |
| 2 | **breakout 레짐 조건부** | 보류 | 상승장 해제 시 +8.3%p, 약세장 MDD 악화 → 레짐 조건부 필요(§22.AO-13) |
| 3 | **pending 변이 16건** | 대기 | 상당수가 무력 키 계열. `max_per_run` 상향 시 소진 가능 |
| 4 | **PEAD 이상치 필터** | 미구현 | 서프라이즈 \|%\|>100 이 **19/808(2.4%)**. EPS 추정치 0 근처 왜곡 |
| 5 | **PEAD 12분기 축적** | 4분기 | 무료 티어 한계. 주간 수집으로 2년 뒤 확보 |
| 6 | **수식 IC 기준 0.02** | 재검토 여지 | 경계 사례(+0.0223/+0.0183) 근소 미달 기각 발생 |
| 7 | **본문 파싱 품질** | 한계 | 병목은 입력이 아닌 3B 모델 능력. 무료 유지 결정으로 현 상태 고정 |
| 8 | **생존편향** | 복원 불가 | 과거 시점 구성종목 목록 부재. 백테스트 수익률 과대평가 요인 |
| 9 | ~~**정수 절삭**~~ | ✅ **해소**(2026-08-27) | 자본 증가를 기다릴 필요가 없었다 — §22.AO-26 B 의 `fractional_shares_enabled` 로 해결. 8/27 이후 진입 9건 **전량 소수 수량**(0.3676~5.7341주), 진입금액 CV 0.529→0.005. "구조적/자본 증가 시 해소"라는 당시 판단은 **틀렸다** |

**자동 운영 (토요일 오전 사이클, 29 잡)**
08:00 research + 수식추출 → 08:30 pead → 09:00 lstm → 10:00 universe →
10:30 factor_ic + 가중치튜닝 → 11:00 자가진단 → 11:30 수식검증

---

### 22.AO-26 수익률 진단 3대 원인 교정 + 1주 관찰 체계 (2026-08-27 적용, 판정 2026-09-10)

> 🔴 **먼저 읽을 것: [§22.AO-26-C 정정](#22ao-26-c)** — 아래 A(출구 지평)의 근거 수치는 **평균**이다.
> 중앙값·승률로 보면 21일 보유가 이긴 비율은 102건 중 **45.1%** 로 논지가 지지되지 않는다(2026-09-14).
...
### 22.AO-26-C 🔴 **정정** — A(출구 지평) 근거는 평균값 편향이었다 (2026-09-14)

§22.AO-26 A 의 "20일만 보유했다면 +0.56% → +4.47%" 는 **평균**이다. 같은 데이터를
중앙값·승률로 다시 보면 **논지가 지지되지 않는다.** 반사실 표본은 그때 65건 → 지금 102건.

| 출구 | n | 현행 평균/중앙 | 21일보유 평균/중앙 | 21일이 나은 비율 |
|---|---|---|---|---|
| rsi2_overbought | 38 | +4.27 / **+4.15** | +9.09 / **+3.99** | 20/38 = **52.6%** |
| atr_trailing_stop | 14 | −0.49 / **−0.30** | +3.22 / **+0.05** | 7/14 = **50.0%** |
| **전체** | **102** | +0.01 / **−0.14** | +1.88 / **−0.61** | 46/102 = **45.1%** |

전체 102건에서 21일 보유가 이긴 비율은 **45.1%** — 동전 던지기보다 못하다. 평균 +1.88% 는
DELL +64.88% 같은 소수 이상치가 만든 값이고, 중앙값은 오히려 **나빠진다**(−0.14 → −0.61).
절사평균(상하 10% 제외)에서도 갭은 rsi2 +2.08pp, trailing +1.78pp 로 절반 이하로 줄어든다.

*평균 갭이 큰 것 자체는 사실이다. 다만 그것은 "보유를 늘리면 대체로 나아진다"가 아니라
"드물게 크게 오르는 종목이 있다"는 뜻이다. 소액·분산 계좌에서 그 꼬리를 잡을 보장은 없다.*

**■ 같이 드러난 것 1 — `atr_trailing_stop` 14건은 전부 브레이크이븐 스톱이었다**
설정은 `atr_trailing_multiplier=2.5` 인데 실제 청산은 고점 대비 1.14~2.77×ATR 에서 일어났다.
청산가를 대조하니 **14건 전부 진입가×1.002(브레이크이븐) 선**이고, 2.5×ATR 트레일링 라인은
언제나 그보다 아래여서 **한 번도 발동한 적이 없다**. 라벨만 `atr_trailing_stop` 이다.
  → `atr_trailing_multiplier` 를 조정했다면 §22.AO-18 과 똑같이 **무효 변경**이 됐을 것이다.
    실제 손잡이는 `breakeven_trigger_r`(=1.0) 다.
  → 다만 브레이크이븐은 **제 역할을 하고 있다**: 이 14건 중 8건이 하드스톱 대상이었고
    하드스톱만 적용 시 −5.17% vs 현행 −0.49% — 4.7pp 낫다. 완화 근거 없음.

**■ 같이 드러난 것 2 — hard_stop 과 trend_break 는 옳게 자르고 있다 (표본 확대 확인)**
| 출구 | n | 실제 | 21일보유 | 갭 |
|---|---|---|---|---|
| hard_stop | **27** (AO-26 당시 11) | −5.16% | −4.91% | **+0.26pp** |
| trend_break | 7 | −2.41% | −6.46% | **−4.06pp** (더 나빠진다) |

**■ 같이 드러난 것 3 — composite_score 는 성과와 단조 관계가 없다**
| 점수 | 청산 | 승률 | 평균수익 | 손익합 |
|---|---|---|---|---|
| 60~65 | 25 | 48.0% | −0.55% | −$27.71 |
| 65~69 | 21 | **76.2%** | +3.28% | +$66.43 |
| 70~75 | 18 | 33.3% | −2.21% | −$58.16 |
| **75~79** | 6 | **0.0%** | **−5.99%** | −$33.29 |
문턱 상향(`composite_score_min` 61→65/70)은 **역효과**다. 최상위 구간이 6전 0승이다.

**■ 같이 드러난 것 4 — 현금 해소는 지금 쓰면 손해다**
평균 투자율 60.6%(현재 40.7%)를 올리자는 안을 검토했으나, 수수료가 **정률**(0.25%/방향)이라
포지션을 키우면 수익과 비용이 **함께** 커진다. 왕복 순기여가 음수인 지금은 손실만 확대된다.
`왕복 순기여 = 총수익 +0.012% − 수수료 0.500% = −0.488%`
  position_pct 0.05 → 0.07 시 연 순수익 −4.11% → **−5.75%**.
  → 거래당 총수익이 왕복 비용 0.50% 를 넘긴 뒤에 쓸 카드다. **적용하지 않았다.**

**■ 결론 — 이번 회차에 바꾼 설정은 없다**
회전율 축소가 비용을 연 5.64% → 2.21% 로 줄이는 것은 산술이라 유효하지만, 그 수단인
**보유기간 연장의 수익 개선 근거가 중앙값에서 사라졌다.** 비용만 줄이고 수익이 그대로면
−5.5% → 약 −2.1% 로, 여전히 목표와 멀다. *진짜 문제는 회전율이 아니라 **엣지가 0** 이라는 것이다*
(거래당 기대값 +0.013%, 손익분기 승률 46.9% vs 실제 47.1%).

⚠️ **교훈**: 평균만 보고 정책을 바꾸면 이상치를 전략으로 착각한다. §22.AO-26 은 그렇게 적용됐고,
이 정정도 같은 데이터를 중앙값으로 한 번 더 본 것뿐이다. **반사실 분석에는 중앙값·승률을 함께 볼 것.**
...
**■ 3. 후속 — 3K 자가진단에 `data_freshness` 추가 (검사 6종 → 7종)**

위 두 건 모두 자가진단이 **6/6 PASS 인 채로** 죽어 있었다. 기존 6종은 전부 "계측이 틀렸는가"를
보는데, 정작 필요한 것은 **"입력이 있는가"** 였다. 그 질문을 하는 검사를 추가했다(severity=critical).

| 검사 항목 | 판정 | 기본 임계값(config 로 조정 가능) |
|---|---|---|
| 매크로 신선도 | 최신 스냅샷이 며칠 전인가 | `self_check_macro_stale_days` = 3 |
| **매크로 컬럼 결손** | 최근 N행이 **전부 null 인 지표** | `self_check_freshness_rows` = 10 |
| 가격 커버리지 | 활성 유니버스 중 최근 5일 바가 있는 비율 | `self_check_price_coverage_min` = 0.90 |
| 가격 바 신선도 | 최신 바가 며칠 전인가(연휴 감안) | `self_check_price_stale_days` = 4 |
| 소셜 수집 | `swing_social_sentiment` 최신 경과일 (일간 06:50) | `self_check_social_stale_days` = 3 |
| PEAD 수집 | `swing_earnings_surprises` 최신 경과일 (주간 토 08:30) | `self_check_pead_stale_days` = 10 |
| **시그널 팩터 입력 회귀** | 뉴스감성·PEAD·LLM 등이 **채워지다가 끊겼는가** | `self_check_signal_window` = 30 |

설계 시 주의한 것:
- **검사 컬럼을 하드코딩하지 않는다.** `information_schema` 에서 숫자 컬럼을 발견해 전부 검사한다 —
  상수로 박으면 지표를 추가할 때마다 사각지대가 생긴다(§22.AO-28 과 같은 함정).
- **커버리지는 유니버스에 속한 심볼만 센다.** `daily_prices` 에는 벤치마크·헤지용 SH 등
  유니버스 밖 심볼이 섞여 있어(226 vs 200), 전체 distinct 로 세면 비율이 1.13 이 나와
  **결손이 있어도 절대 FAIL 하지 않는다.** 초안에서 실제로 이 실수를 했다가 실측으로 잡았다.
- 부분 결손(`macro_partial`)은 경보하지 않고 detail 에만 남긴다 — 복구 직후 구간이 여기 잡힌다.
- **시그널 팩터 입력은 '결측'이 아니라 '회귀'를 본다.** 최근 30건과 직전 60건의 채움률을 비교해
  `직전 ≥50% → 최근 <50%` 인 컬럼만 잡는다. 단순 결측률로 잡으면 두 가지가 오탐이 된다:
  **사장된 컬럼**(`tech_score` — 213건 내내 0, 아무 코드도 안 쓴다)과
  **신규 도입분**(`pead_score` — 14/60 → 30/30 으로 증가 중). 제외 목록을 하드코딩하면 낡는다.

검증:
- **소급 검증** — 어제(2026-09-13) 시점의 최근 10행으로 돌리면 `dxy` 0/10, `dxy_momentum_20d` 0/10
  → 🔴 FAIL. **이 검사가 있었다면 DXY 6개월 결손을 잡았다.**
- FAIL 3경로(매크로 신선도·커버리지·바 신선도)를 임계값 일시 조정으로 전부 발화 확인 후 원복
  (테스트 config·자가진단 행 전량 삭제 확인).
- 주말·휴장 오탐 없음 — 9/11(금) 최신 바, 9/14(월) 기준 `price_stale_days=3` 으로 통과.
  9/7 은 미국 노동절 휴장이라 결손이 아니다.
- 회귀 탐지는 **창을 뒤집어 실데이터로 재현** — `pead_score` 채움률 100%→0% 로 놓으면 🔴 탐지,
  사장된 `tech_score` 는 0%→0% 라 조용함(의도대로).
- 소셜·PEAD 신선도 FAIL 경로도 임계값 일시 조정으로 발화 확인 후 원복
  (테스트 config·자가진단 이력 행 전량 삭제 확인).
- 엔진 재시작 후 실 API 실행: **7/7 PASS** (critical 4종 포함), 하위 7항목 전부 보고.
...
```
89dc5aa  feat(harness): 3I 팩터 가중치 자동 튜닝 — 구조적 사각지대 제거
95d19a0  fix(ai): 가중치 교정이 무효였음 — REGIME_WEIGHTS 수정 + ①③ 포트폴리오 적용
4295de5  feat(analysis): ①③ 자동 검증 판정 — 알림 루프 완성
195afe3  feat(analysis): 관찰 체계 — 주간 팩터 IC 자동 측정
b8a7f4a  analysis: ④ 마이크로구조 17종 기각 — 재현 실패
d26044b  feat(strategy): ③ PEAD 구축 — Layer 3 placeholder 채움
106360e  analysis: breakout 음수 기여 진단 — 보유기간에 따라 부호 반전
6491460  feat(strategy): ② 신호 교집합 게이트 — 승률 66.2% → 76.4%
8b2b0f4  feat(analysis): 시그널 리플레이 — flow/value 가 음의 IC
7465816  feat(ai): ① LLM 모멘텀 조건화 — naive 감성질의를 정합성 판단으로
80c327d  docs: 새 엣지 리서치 — 교과서 팩터는 사망, 살아있는 엣지 4종
89b3eb2  analysis: 백테스트↔라이브 격차 분해 + 팩터 가중치 교정
711bbf3  docs+test: v42 기각(정수 절삭 부산물) + 멀티팩터 이식 실현가능성 판정
09efb0e  docs: project_status 갱신 — §25 Tier 수치 정정 + §23/§24 현행화
...
- 🔴 **반사실·백테스트는 평균만 보지 말 것 — 중앙값과 승률을 함께 볼 것**(2026-09-14 교훈, §22.AO-26-C).
  ...
  **평균만 보고 정책을 바꾸면 이상치를 전략으로 착각한다.**
- 🔴 **수수료가 정률이면 '현금 해소'는 중립이 아니다**(2026-09-14, §22.AO-26-C).
  포지션을 키우면 수익과 비용이 함께 커진다. 왕복 순기여(총수익 − 왕복수수료)가 음수인 동안은
  자본을 더 투입할수록 손실이 커진다. 현재 `+0.012% − 0.500% = −0.488%`.
...
- 🔴 **수치 보고 전 ① 구간 길이 ② 적용 경로를 확인할 것.** 2026-08-19 세션에서 두 번 틀렸다:
  팩터 가중치 "적용 완료"가 REGIME_WEIGHTS 하드코딩에 막혀 무효였고(§22.AO-18),
  "연율 8.9% 상한"은 5.5개월 구간 연율화 오류였다(4.6년 기준 21.84%, §22.AO-20).
- 🔴 **워치리스트 ≠ 포트폴리오.** 수익은 오직 `scan → swing_signals → auto_approve →
  swing_positions` 경로에서만 발생한다. `watchlist_strategy.py` 수정은 수익률에 영향 없음(§22.AO-18).
