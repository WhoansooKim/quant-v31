...
├── ai/
│   ├── sentiment.py          # SentimentAnalyzer (Claude Haiku 4.5 or mock) [Phase A]
...
| Phase A: LLM Sentiment | COMPLETE | AI 분석 (Mock+Live Claude), Signals 페이지 AI Score 표시 |
...
| Help Page (Korean) | COMPLETE | 10섹션 전체 한글 가이드 (시간외 거래 + TWR 설명 포함) |
...
| **Watchlist** | `/watchlist` | `Watchlist.razor` | 보유종목 등록 + 가중 스코어링 + 3단계 접기 + 시그널 백테스트 + 인트라데이 차트 |
| Settings | `/settings` | `Settings.razor` | swing_config 편집 + Capital Management (deposit/withdraw) |
| **Help** | `/help` | `Help.razor` | 전체 시스템 가이드 (한글, 10개 섹션) |
...
├── docker-compose.yml
├── .env                                # API 키 (ANTHROPIC_KEY, FINNHUB_API_KEY, TELEGRAM_*, KIS_*)
│
...
│   ├── ai/
│   │   ├── sentiment.py                # SentimentAnalyzer (Claude + mock)
...
│   │       ├── Watchlist.razor         # 보유종목 등록 + 기술분석 추천
│   │       ├── Settings.razor          # 설정 + Capital Management
│   │       └── Help.razor              # 전체 가이드 (한글, 9개 섹션)
│   └── wwwroot/
...
| 키 | 용도 | 상태 |
|----|------|------|
| `ANTHROPIC_KEY` | Claude API (감성분석, 최적화) | 설정 필요 (mock 모드 동작) |
...
9. **New features**: Capital Injection, Watchlist(weighted scoring + signal backtest + intraday chart), Collapsible Sidebar(JS+localStorage), Live Ticker, Help(Korean), Extended Hours, Performance TWR Fix, CNN Ticker Links
