...
- **Open Positions**: 5개 (FANG, VLO, ROST, MRVL, LYB)
...
| **Help** | `/help` | `Help.razor` | 전체 시스템 가이드 (한글, 12개 섹션, 용어사전 + 초보자 교육 + Tier 3 통합) |
...
## 6. Scheduler Jobs (APScheduler, KST 기준 — 11개)
...
| watchlist_analysis | 월~토 07:40 | 워치리스트 자동 24지표 분석 | No |
...
## 7. Database Schema (swing_* tables — 17개)
...
│   │   ├── multi_factor.py             # MultiFactorScorer (5-Factor 레짐 적응형)
│   │   ├── lstm_predictor.py           # LSTMPredictor (LSTM 딥러닝, 70.5%)
│   │   ├── social_sentiment.py         # SocialSentimentCollector (Reddit+StockTwits)
│   │   ├── fundamental.py              # FundamentalAnalyzer (Quality+Value)
│   │   ├── factor_crowding.py          # FactorCrowdingMonitor (밀집도 경고)
...
│   ├── scheduler/jobs.py               # SwingScheduler (11 jobs + generate_snapshot)
...
│   │       └── Help.razor              # 전체 가이드 (한글, 12개 섹션, 용어사전+초보자 교육)
│   └── wwwroot/
...
```
PENDING  feat: Tier 3 (LSTM + Social Sentiment + Dual Sort) + Help 재구성
