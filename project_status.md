...
### Phase A: LLM Sentiment Overlay (즉시 구현 가능)
...
### Phase B: Exit 전략 고도화 (규칙 기반, LLM 불필요)
...
```
Phase A: LLM Sentiment Overlay ─────── [우선순위 1]
  A-1. DB 마이그레이션
  A-2. SentimentAnalyzer 모듈
  A-3. API 엔드포인트
  A-4. Dashboard 표시
  A-5. 테스트 + 검증

Phase B: Exit 전략 고도화 ──────────── [우선순위 2]
  B-1. TrailingStopManager
  B-2. PartialExitManager
  B-3. Config 파라미터 추가
  B-4. 스케줄러 통합
  B-5. 백테스트로 효과 검증
...
8. **Next work**: Section 14의 Phase A부터 순서대로 진행
