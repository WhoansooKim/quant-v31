...
Multi-Factor Score: Technical(40%) + Sentiment(30%) + Flow(30%) ≥ 50점
Exit: Trailing Stop(-3% from high) + Partial Exit(50% at +7%) + SL -5% / TP +10%
...
```
engine_v4/
├── api/main.py               # FastAPI (port 8001) — 전체 API + 스냅샷 + 워치리스트 분석 + 시간외
...
│   ├── data_feeds.py         # FinnhubClient (news/insider/earnings/recs) [Phase C]
│   ├── multi_factor.py       # MultiFactorScorer (T40+S30+F30) [Phase C]
...
│   └── exit_manager.py       # ExitManager (TrailingStop + PartialExit) [Phase B]
├── broker/kis_client.py      # KIS 증권 API (paper=SIM / live=KIS API)
├── scheduler/jobs.py         # APScheduler (8 jobs, KST 기준)
...
| Watchlist | COMPLETE | 24지표 + 가중 카테고리 스코어링 (레짐 적응형 + 볼륨 확인) + 3단계 접기 |
| Collapsible Sidebar | COMPLETE | 아코디언 사이드바 (접으면 아이콘만 표시) |
| Ticker Bar (Live) | COMPLETE | 오픈 포지션 심볼의 현재가 실시간 표시 |
| Help Page (Korean) | COMPLETE | 15섹션 전체 한글 가이드 (Ollama AI + 리플레이 백테스트 + 시간외 거래 + TWR) |
...
- **Open Positions**: 4개 (FANG, VLO, ROST, LYB)
- **Position Sizing**: 20% per position ($200)
- **Price Range Filter**: $10 ~ $60
- **Max Positions**: 3

---

## 4. Dashboard Pages (10 pages + Login)
...
| **Watchlist** | `/watchlist` | `Watchlist.razor` | 보유종목 등록 + 가중 스코어링 + 3단계 접기 + 시그널 백테스트 + 인트라데이 차트 + US Market Sector Heatmap (드릴다운) |
| Settings | `/settings` | `Settings.razor` | swing_config 편집 + Capital Management (deposit/withdraw) |
| **Help** | `/help` | `Help.razor` | 전체 시스템 가이드 (한글, 15개 섹션, Ollama AI + 리플레이 백테스트) |

### Nav Menu 순서
Dashboard → Pipeline → Signals → Performance → Positions → Backtest → Events → **Watchlist** → Settings → Help
...
| NavMenu | `Layout/NavMenu.razor` | 10 nav items + pending badge + **접기/펼치기 토글** |
...
## 6. Scheduler Jobs (APScheduler, KST 기준 — 8개)
...
## 7. Database Schema (swing_* tables — 13개)
...
| swing_positions | regular | 포지션 (open/closed, entry/exit/pnl) + trailing_stop_active, partial_exited, high_water_mark |
...
factor_weight_technical  = 0.4        ← 기술지표 가중치 40%
factor_weight_sentiment  = 0.3        ← 감성분석 가중치 30%
factor_weight_flow       = 0.3        ← 수급/이벤트 가중치 30%
trading_mode             = paper
...
## 9. Watchlist System (가중 카테고리 스코어링 + 3단계 접기)

### 개요
기존에 보유 중인 종목을 등록하고, 24개 기술지표를 5개 카테고리로 분류하여 가중 스코어링으로 매수/매도/보유 추천을 받는 독립 기능. Pipeline/Signals 시스템과는 별개로 동작.

### 기술분석 지표 (24개, 5개 카테고리)
| 카테고리 | 기본 가중치 | 지표 |
|----------|-----------|------|
| Trend (추세) | 30% | SMA20/50, SMA50/200, ADX 방향, 가격 vs SMA200 |
| Momentum (모멘텀) | 25% | RSI(14), Stochastic(14,3), ROC(10), Williams %R |
| MACD | 20% | MACD 크로스, 히스토그램 방향, 시그널 거리 |
| Mean Reversion (평균회귀) | 15% | BB 위치, BB %B, RSI 극단, 가격 vs SMA20 |
| Volume (거래량) | 10% | 거래량 비율, OBV 추세, 거래량 가격 확인 |

### 레짐 적응형 가중치 (ADX 기반)
```
ADX > 25 (TRENDING): Trend 40%, Momentum 25%, MACD 20%, MeanRev 10%, Vol 5%
ADX < 20 (SIDEWAYS): Trend 15%, Momentum 20%, MACD 15%, MeanRev 30%, Vol 20%
기타 (MIXED):        기본 가중치 유지
```

### 볼륨 확인 (Volume Confirmation)
- `vol_ratio < 1.5` → 최종 점수 50% 감쇄 (볼륨 미확인 시그널 약화)

### 최종 판정 임계값
```
weighted_score ≥ +0.35  → STRONG BUY
weighted_score ≥ +0.12  → BUY
weighted_score ≤ -0.35  → STRONG SELL
weighted_score ≤ -0.12  → SELL
기타                     → NEUTRAL
```

### 프리마켓 볼륨 부스트 (C안)
- 프리마켓 가격 > 전일 종가 +1% → Volume 카테고리 가중치 50% 추가 부스트
- 결과에 `pre_vol_boost` 필드로 부스트 적용 여부 표시

### 3단계 접기 카드 (Collapsible)
| 레벨 | 표시 내용 | 기본 상태 |
|------|----------|----------|
| Level 1 | 심볼(CNN 링크) + 회사명 + 현재가 + P&L% + 판정 배지 + 점수 | 항상 표시 (클릭으로 펼치기) |
| Level 2 | 좌: Summary Gauge (판정 + 카테고리별 점수 바), 우: 인트라데이 차트 (프리/정규/애프터) | 클릭 시 펼침 |
...
│   │   ├── data_feeds.py               # FinnhubClient (news/insider/earnings/recs)
│   │   ├── multi_factor.py             # MultiFactorScorer (T40+S30+F30)
...
│   │   └── exit_manager.py             # ExitManager (TrailingStop + PartialExit)
│   ├── broker/kis_client.py            # KIS API (paper: SIM-* / live: KIS API)
│   ├── scheduler/jobs.py               # SwingScheduler (8 jobs + generate_snapshot)
...
│   │       ├── Settings.razor          # 설정 + Capital Management
│   │       └── Help.razor              # 전체 가이드 (한글, 15개 섹션)
│   └── wwwroot/
...
2. 장중 청산 체크 (자동, 3회/일)
   ...
   ※ Trailing Stop + Partial Exit 자동 적용

3. 워치리스트 (수동)
   ├─ Watchlist 페이지 → 보유종목 등록
   ├─ "Analyze All" 클릭 → RSI/MACD/BB/SMA/ATR 기술분석
   ├─ BUY/SELL/HOLD 추천 + 신뢰도 표시
...
```
PENDING  feat: Ollama local LLM, background AI analysis, market sector heatmap, Help 15-section
...
3. **Current state**: V4 Swing Trading 시스템 운영 중, 5개 포지션 오픈 (paper mode)
4. **Dashboard**: 10 pages + Login + Collapsible Sidebar + Live Ticker Bar
5. **Engine**: port 8001 (NOT 8000), 8 scheduler jobs (incl. premarket/afterhours), SSE stream
6. **Key service**: `SwingService.cs` (NOT PostgresService.cs)
7. **Key models**: `SwingModels.cs` (NOT DashboardModels.cs)
8. **All phases complete**: A(LLM) + B(Exit) + C(MultiFactor) + D(Optimizer) + E(Events) + F(Telegram/KIS)
9. **New features**: Capital Injection, Watchlist(weighted scoring + signal backtest + intraday chart + sector heatmap), Collapsible Sidebar(JS+localStorage), Live Ticker, Help(Korean 15섹션), Extended Hours, Performance TWR Fix, CNN Ticker Links, Ollama Local LLM, Background AI Analysis, Market Sector Heatmap(in-place drilldown), Mobile Responsive, Pagination, Chart Touch Zoom, Signal Replay Backtest
