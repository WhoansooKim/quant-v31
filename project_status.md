...
| **Watchlist** | `/watchlist` | `Watchlist.razor` | 보유종목 등록 + 가중 스코어링 + 3단계 접기 카드 |
...
### UI Features
- **Collapsible Sidebar**: 아코디언 방식 접기/펼치기, 접혔을 때 아이콘만 표시
...
| 레벨 | 표시 내용 | 기본 상태 |
|------|----------|----------|
| Level 1 | 심볼 + 회사명 + 현재가 + P&L% + 판정 배지 + 점수 | 항상 표시 (클릭으로 펼치기) |
| Level 2 | Summary Gauge (판정 + 카테고리별 점수 바) | 클릭 시 펼침 |
...
9. **New features**: Capital Injection, Watchlist(weighted scoring), Collapsible Sidebar, Live Ticker, Help(Korean), Extended Hours, Performance TWR Fix
