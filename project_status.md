...
- **Open Positions**: 1개 (MRVL 1주 @ $128.49)
...
```
PENDING  feat: KIS broker 완전 연동 + daily entry count 버그 수정 + Scan Now 탭 유지
...
- **KIS Broker**: python-kis 2.1.6, paper→시뮬레이션(모의앱키 없으면), live→실전 KIS API 연결
  - 실전 앱키: `KIS_APP_KEY`/`KIS_APP_SECRET` (필수)
  - 모의 앱키: `KIS_VIRTUAL_APP_KEY`/`KIS_VIRTUAL_APP_SECRET` (paper KIS 연결 시 필요)
  - `KIS_USER_ID`: HTS 로그인 ID (필수)
  - Broker API: /broker/status, /broker/balance, /broker/quote, /broker/sync 등 8개
