...
## 9. Watchlist System (Connors RSI(2) 평균회귀 전략)

### 개요
대형 기술주(Mag 7 등)를 위한 독립 모니터링 기능. Larry Connors의 RSI(2) 평균회귀 전략 기반, 1주~15일 단기 매수/매도 타이밍 감지. (연간 수익률 26.6%, 승률 75%)

### 핵심 지표 (8개)
| 지표 | 역할 | 매수 조건 |
|------|------|-----------|
| RSI(2) | 초단기 과매도 감지 | < 5 |
| CumRSI(2) | 2일 누적 RSI(2) | < 5 |
| RSI(14) | 중기 추세 참고 | < 50 |
| Stochastic(5,3,3) | 단기 가격 위치 | < 20 |
| MACD Hist | 모멘텀 방향 | 양수 전환 |
| MFI(14) | 자금흐름 (거래량 가중) | < 30 |
| ADX(14) | 추세 강도/레짐 분류 | - |
| BB Squeeze | 변동성 수축 감지 | bandwidth 10th percentile |

### 5개 카테고리 가중치
| 카테고리 | 기본 가중치 | 강추세(ADX>25) | 횡보(ADX<20) |
|----------|-----------|---------------|-------------|
| RSI(2) Reversion | 35% | 30% | 40% |
| Trend | 25% | 35% | 15% |
| Volume | 15% | 10% | 15% |
| Rel. Strength (vs QQQ) | 15% | 15% | 15% |
| Volatility | 10% | 10% | 15% |

### 핵심 필터
- **SMA(200) 필터**: 종가 < SMA(200) → BUY 스코어 70% 감쇠
- **Connors 청산**: 종가 > SMA(5) → EXIT 신호
- **OPEX 주간 필터**: 매월 셋째 금요일 주간 → BUY 스코어 50% 감쇠
- **QQQ 상대강도**: 5일/20일 수익률 vs QQQ 비교

### 최종 판정 임계값
```
weighted_score ≥ +0.40  → STRONG BUY
weighted_score ≥ +0.15  → BUY
weighted_score ≤ -0.40  → STRONG SELL
weighted_score ≤ -0.15  → SELL
기타                     → NEUTRAL
```

### 추가 출력 필드
- `rsi2`, `cum_rsi2`, `connors_exit`, `opex_week`, `rel_str_5d`, `rel_str_20d`, `bb_squeeze`
